...
public class TestIterator {
  ...
  public void testRemove() {
    ...
    while (i.hasNext()) {
      if (i.next() == 77) {
        i.remove();
        // (complete) TODO what happens if you use list.remove(Integer.valueOf(77))?
      }
    }
    // (complete) TODO using assertEquals and List.of, express which values are left in the list
    // See TestList.java for examples of how to use List.of; also see the Java List
    // interface for more information
    assertEquals(List.of(33, 44, 55, 66), list);
  }
  // Implementation of testRemove() that uses list.remove(Integer.valueOf(77)).
  @Test
  public void testRemove2() {
    ...
    list.add(77);
    list.add(66);
    while(list.remove(Integer.valueOf(77))) {
  ...
  public void testAverageValues() {
    ...
    double sum = 0;
    int n = 0;
    // (complete) TODO use an iterator and a while loop to compute the average (mean) of the values
    final var i = list.iterator();
    n = list.size();
    while(i.hasNext()) {
      sum = sum + i.next();
